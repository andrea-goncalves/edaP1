...
void gravarEquipa(const Equipa& equipa, const int jornada, const string& nomeFicheiro) {
    ...
    out.close();
    cout << "[INFO] Equipa e estado do campeonato gravados com sucesso!\n";
}

bool carregarEquipa(Equipa& equipa, int& jornada, const string& nomeFicheiro) {
    ifstream in(nomeFicheiro);
    if (!in.is_open()) {
        cout << "[ERRO] Nao foi possivel encontrar o ficheiro: " << nomeFicheiro << "\n";
        return false;
    }
    ...
    in.close();
    cout << "[INFO] Equipa e campeonato carregados! Bem-vindo de volta a jornada " << jornada << "!\n";
    return true;
}